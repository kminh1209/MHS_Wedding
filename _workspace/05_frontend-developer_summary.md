...
| Kakao JS SDK / Maps SDK | (선택) | venue 카카오맵은 `script.js`에서 동적 로드, 카카오 공유는 별도 SDK 로드 시 활성화 |
...
|------|----------------|--------|------------|
| Kakao JS SDK 키 | `script.js` `KAKAO_JS_KEY` | `''` | https://developers.kakao.com/ 에서 JavaScript 키 발급 후 입력. venue 카카오맵은 즉시 사용되고, 카카오 공유는 하단 SDK 로드와 함께 활성화됩니다. |
...
| 지도 UI / 대체 SDK | `index.html` `#map`, `script.js` `setupVenueMap()` | 카카오맵 SDK + 카카오맵 바로가기 폴백 | `KAKAO_JS_KEY`가 비어 있으면 카드 내부에 카카오맵 바로가기만 표시됩니다. 키를 입력하면 venue 섹션에 카카오 지도가 렌더링됩니다. |
